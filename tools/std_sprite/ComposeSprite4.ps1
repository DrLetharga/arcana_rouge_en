...
function Resolve-Filenames {
    ...
    $faceCodeMaps = @{
        "mei" = @{ 'ha' = 'a'; 'si' = 'a'; 'siw' = 'a'; 'ja' = 'a'; 'jaw' = 'a'; 'ma' = 'c'; 'mad' = 'c'; 'maw' = 'c'; 'madw' = 'c' }
        # Add additional maps for aka, aoi, mia as needed
    }
    ...
    # ---- Face Filename ----
    $faceCodeMap = $faceCodeMaps[$parsed.character]
    $facePrefix = $faceCodeMap[$parsed.outfit]
    $blushOffset = if ($parsed.blush) { 1 } else { 0 }
    $faceNum = ($faceID * 10) + $blushOffset

    # ---- Face Filename ----
    $face = "${charCode}_${size}_face_${facePrefix}0_${pose}_{0:D3}.png" -f $faceNum
    ...
    $addon1 = $null
    $addon2 = $null
    $semenOverlay = $overlay

    if ($parsed.character -eq 'mei') {
        # Addon1: p2 + magic outfit
        if ($parsed.pose -eq 'p2' -and $parsed.outfit -in @('ma', 'mad', 'maw', 'madw')) {
            $addon1 = "ch04_${size}_face_f${eki}_p2_add.png"
        }
        # Addon2: p2 + casual outfit
        elseif ($parsed.pose -eq 'p2' -and $parsed.outfit -in @('ha', 'si', 'siw', 'ja', 'jaw')) {
            $addon2 = "ch04_${size}_face_a${eki}_p2_add2.png"
        }
    }

    return @{ body = $body; face = $face; addon1 = $addon1; addon2 = $addon2; semenOverlay = $semenOverlay }
}
...
function Compose-Image {
    ...
    if ($filenames.body) {
        $bodyPath = Join-Path $scriptRoot "..\source\g_std\$charCode\$($filenames.body)"
        Write-Host "Body: $bodyPath"
        $layers += "`"$bodyPath`" -geometry +$($pos.x)+$($pos.y)"
    }
    if ($filenames.face) {
        $facePath = Join-Path $scriptRoot "..\source\g_std\$charCode\$($filenames.face)"
        Write-Host "Face: $facePath"
        $layers += "`"$facePath`" -geometry +$($pos.x)+$($pos.y)"
    }
    if ($filenames.addon1) {
        $addon1Path = Join-Path $scriptRoot "..\source\g_std\$charCode\$($filenames.addon1)"
        Write-Host "Addon1: $addon1Path"
        $layers += "`"$addon1Path`" -geometry +$($pos.x)+$($pos.y)"
    }
    if ($filenames.addon2) {
        $addon2Path = Join-Path $scriptRoot "..\source\g_std\$charCode\$($filenames.addon2)"
        Write-Host "Addon2: $addon2Path"
        $layers += "`"$addon2Path`" -geometry +$($pos.x)+$($pos.y)"
    }
    if ($filenames.semenOverlay) {
        $overlayPath = Join-Path $scriptRoot "..\source\g_std\$charCode\$($filenames.semenOverlay)"
        Write-Host "Semen: $overlayPath"
...
# Example Use
# Single-line test
$testMacro = "[std_mei m mad p2 f12 eki ho ce]"
$parsed = Parse-Macro -macro $testMacro
$filenames = Resolve-Filenames -parsed $parsed
Compose-Image -filenames $filenames -outputPath $outputPath -location $parsed.location

# Batch-mode example
# Process-MacroBatch -inputFile ".\macros.txt" -outputPath $outputPath -defaultBackground ".\g_bg\bg001a.png"
