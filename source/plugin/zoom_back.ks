@if exp="typeof(global.zoom_back_object) == 'undefined'"
@iscript
...
class ZoomPlugin extends KAGPlugin
{
	...
	function startZoom(storage, layer, mode, basestorage, sl, st, sw, sh, dl, dt, dw, dh, time, accel)
	{
		...
		if(layer == '' || layer == 'base')
			...
			targetLayer = window.back.base;
		else
			//targetLayer = window.fore.layers[+layer];
			targetLayer = window.back.layers[+layer];

		// 背景画像の読み込み
		if(basestorage !== void)
			//window.tagHandlers.image(%[ storage : basestorage, layer : layer, page : 'fore']);
			window.tagHandlers.image(%[ storage : basestorage, layer : layer, page : 'back']);
	...
	function finish()
	{
		...
		if(targetLayerName == 'base')
		{
				...
				tempLayer.imageWidth == base.imageWidth && tempLayer.imageHeight == base.imageHeight)
			{
				...
				window.tagHandlers.image(%[ storage : storage, layer : 'base', page : 'back']);
			}
			else
			{
				// 最終位置が画面全体を覆っていない;
				...
				window.back.base.face = dfBoth;
				//window.fore.base.stretchCopy(dl, dt, dw, dh, tempLayer, 0, 0,
				window.back.base.stretchCopy(dl, dt, dw, dh, tempLayer, 0, 0,
		...
		else
		{
			...
			if(dw == tempLayer.imageWidth && dh == tempLayer.imageHeight)
			{
				...
				//mm ここbackにしたらtransできる？
				window.tagHandlers.image(%[ storage : storage, layer : targetLayerName, page : 'back',
...
kag.addPlugin(global.zoom_back_object = new ZoomPlugin(kag));
	// プラグインオブジェクトを作成し、登録する

@endscript
@endif
; マクロ登録
@macro name="bgzoom_back"
@eval exp="zoom_back_object.startZoom(mp.storage, 'base', 'rect', mp.basestorage, +mp.sl, +mp.st, +mp.sw, +mp.sh, +mp.dl, +mp.dt, +mp.dw, +mp.dh, +mp.time, +mp.accel)"
@endmacro
@macro name="wbgzoom_back"
@if exp="zoom_back_object.moving"
@waittrig * name="zoom" onskip="zoom_object.finish()"
@endif
@endmacro
@macro name="fgzoom_back"
@eval exp="zoom_back_object.startZoom(mp.storage, mp.layer, mp.mode, void, +mp.sl, +mp.st, +mp.sw, +mp.sh, +mp.dl, +mp.dt, +mp.dw, +mp.dh, +mp.time, +mp.accel)"
@endmacro
@macro name="wfgzoom_back"
@if exp="zoom_back_object.moving"
@waittrig * name="zoom" onskip="zoom_object.finish()"
@endif
@endmacro
