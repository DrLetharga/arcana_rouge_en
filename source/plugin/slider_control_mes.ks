...
{
	var tabLayer02;			// ツマミレイヤ
	var valueLayer02;			// 現在値表示用レイヤ
	var mouseOn = false;	// マウスがスライダーコントロール上にあるか否か

	var sliderMin02 = 0;			// スライダーの最小値
	var sliderMax02 = 100;		// 同、最大値
	var sliderPosition02 = 0;		// 同、現在置
	var unitName02 = "";			// 単位名
	var onChangeFunction02 = void;	// スライダーの値が変わったときに呼び出される関数
	...
	var tabLeftMax = sf.srd_mes_tableftmax;					// 同、Ｙ座標

//	var valueOffsetX = 230;					// 現在値表示用レイヤのＸ座標オフセット（ピクセル）
//	var valueOffsetY = 10;					// 同、Ｙ座標オフセット（ピクセル）
